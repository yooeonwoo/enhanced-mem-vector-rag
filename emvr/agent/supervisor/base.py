...
import os
import uuid
from typing import Any, Dict, List, Optional, TypedDict, Union, Annotated, Callable
...
class SupervisorAgent(BaseAgent):
                ...
                if message.get("tool_calls"):
                    for tool_call in message.get("tool_calls", []):
                        if tool_call.get("name", "").startswith("transfer_to_"):
                            ...
                            next_agent = tool_call.get("name").replace("transfer_to_", "")
                            break
    ...
    def _create_worker_node(self, agent_name: str, agent: BaseAgent) -> Callable:
        ...
        def worker_node(state: SupervisorAgentState) -> Dict[str, Any]:
            ...
            if not user_messages:
                ...
                return add_messages(
                    state, 
                    [{"role": "system", "content": "No user message found. Returning to supervisor."}]
                )
    ...
    def _create_handoff_tool(self, agent_name: str) -> Tool:
        ...
        description = getattr(agent, "description", f"Specialist agent for {agent_name} tasks")
        
        @tool(f"transfer_to_{agent_name}", description=f"Transfer task to {agent_name}. {description}")
